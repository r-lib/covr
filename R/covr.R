...
#' @param no_try whether to omit wrapping test evaluation in a \code{try} call;
      ...
      flags, {
        if (use_subprocess) {
          subprocess(
                     clean = clean,
                     quiet = quiet,
                     coverage <- run_tests(pkg, tmp_lib, dots, type, quiet, no_try=no_try)
                     )
        } else {
          coverage <- run_tests(pkg, tmp_lib, dots, type, quiet, no_try=no_try)
        }
...
run_tests <- function(pkg, tmp_lib, dots, type, quiet, no_try=FALSE) {
