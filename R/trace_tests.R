...
#'
#' 1. If any `srcref` files are provided by a relative filepath, assume the test
#'    traces are generated by a testing suite which executes using the `/tests`
#'    directory as a working directory, such as [testthat].
#' 1. Otherwise, if any `srcref` files are are provided by a file within
#'    [covr]'s temporary library, all calls from those files are kept as a test
#'    trace. This will collect traces from tests when using [RUnit].
#' 1. Finally, as a conservative fallback in situations where neither of these
#'    criteria are satisfied, the entire call stack is captured.
#'
