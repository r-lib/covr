...
`package_coverage` performs coverage calculation on an R package.
Unit tests must be contained in the `"tests"` directory.
...
The clear box view can be accessed using online tools or **shiny**, using `shine`.
...
[**testCoverage**](https://github.com/MangoTheCat/testCoverage) by Tom Taverner, Chris Campbell & Suchen Jin were the two of which I was most aware.
...
`getParseData`) is analyzed to impute source references at sub-statement level for `if`, `for` and `while` constructs.
